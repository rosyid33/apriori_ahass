...
if ($num_rows > 0) {
    $rows = $db->db_fetch_array($result);
        ...
        $_SESSION['apriori_ahass_id'] = $rows['id']; // mengisi session
        $_SESSION['apriori_ahass_username'] = $rows['username'];
        $_SESSION['apriori_ahass_level'] = $rows['level'];

        $level_name = ($_SESSION['apriori_ahass_level']==1)?"admin":"kepala";
        $_SESSION['apriori_ahass_level_name'] = $level_name;
        $_SESSION['apriori_ahass_key'] = sha1(date("Y-m-d H:i:s") . $rows['id']);
        $_SESSION['apriori_ahass_last_login'] = date("d-m-Y H:i:s");
